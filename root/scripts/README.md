...
* **Бэкап базы данных** - регулярный дамп базы данных PostgreSQL и отправка дампа по электронной почте администратору.
...
При создании объекта "Скрипт" заполняется поле slug, который используется для создания абсолютного URL, по которому вызывается запуск скрипта. Допустим, у скрипта slug `destroy-all-humans`. Тогда абсолютный путь для запуска данного скрипта будет выглядеть следующим образом: `https://<domain_name>/<app_name>/destroy-all-humans`.
...
Запуск скрипта по расписанию можно настроить через cron, используя команду `curl <URL>`, где URL - это полный путь для запуска скрипта. Вручную запустить скрипт можно через административный интерфейс, где также доступны просмотр истории запусков и логи.