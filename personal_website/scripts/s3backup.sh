...
# Проверить тип хранилища и выйти, если используется S3
echo "Текущее значение STORAGE_TYPE: '$STORAGE_TYPE'"
if [ "$STORAGE_TYPE" = "s3" ]; then
    echo "Файловое хранилище уже находится в S3. Бэкап не требуется."
    exit 0
fi
