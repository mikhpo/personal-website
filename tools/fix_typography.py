...
кавычки-ёлочки (U+00AB и U+00BB) - на прямые кавычки. Запрещённые символы
заданы байтовыми последовательностями, поэтому сам файл скрипта правило
не нарушает. Без аргументов обрабатывает все файлы, отслеживаемые git,
с аргументами - только переданные. Обрабатываются только файлы,
декодируемые как UTF-8; остальные пропускаются без изменений. Возвращает
код 1, если хотя бы в одном файле выполнена замена, иначе - 0.
"""
