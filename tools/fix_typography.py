...
файл скрипта правило не нарушает. Вызывается хуком pre-commit
fix-typography; без аргументов обрабатывает все файлы, отслеживаемые git.
При внесении хотя бы одной замены возвращает код 1: коммит отклоняется,
исправленные файлы нужно добавить в индекс (git add) и повторить коммит.
"""
...
def replace_typography(path: Path) -> bool:
    ...
    """
    # Симлинк пропускается: перезапись записала бы вместо него обычную копию
    # файла (CLAUDE.md и QWEN.md - симлинки на AGENTS.md).
...
def main() -> int:
    ...
    fixed = [path for path in collect_files() if replace_typography(path)]
