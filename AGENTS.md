...
Комбинированная стратегия: stdout/stderr контейнеров собирает Docker (json-file, ротация 10 МБ x 3 файла через якорь `x-logging` в compose.yaml), детальные логи приложения пишутся в volume `${LOGS_ROOT:-./logs/}`. Ошибки Django уровня ERROR дублируются в stdout через handler `console_prod`.
