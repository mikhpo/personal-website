...
- Для локальной разработки обычно достаточно профиля postgres с файловым хранилищем; minio - при разработке с S3-хранилищем, nginx - при отладке прокси-слоя
...
- В окружениях с внешней инфраструктурой переменную `COMPOSE_PROFILES` не задавать; диагностические команды адресовать только сервисам application и nginx (`up -d postgres` поднял бы сервис профиля postgres)
...
- Декомпозировать функции по модулям по назначению: `utils.py` - вспомогательные функции, которые вычисляют и подготавливают данные, но не выполняют операций; `services.py` - функции, выполняющие операции
...
- Писать тесты на базе django.test (TestCase, SimpleTestCase)
- Покрывать все нетривиальные функции модуля: если функция копирует файлы - проверять реальное копирование, если у функции есть флаги - проверять их работу
- Не использовать patch/mock (unittest.mock) в тестах: поведение проверяется на реальных операциях и чистых функциях; если тест нельзя спроектировать без подмены функций - он бесполезен и удаляется
- Не манипулировать переменными окружения через os.environ в тестах: конфигурация задается через override_settings (или self.settings для динамических значений); тесты, которые нельзя передалать на override_settings, удаляются
...
### Ограничения запуска скриптов деплоя

- Не запускать скрипты деплоя (scripts/deploy.sh, scripts/docker/deploy.sh, scripts/server/deploy.sh) на локальной машине разработчика: они переключают рабочую копию на main, останавливают стек командой docker compose down и удаляют неиспользуемые образы Docker всей машины (image prune)
- Скрипты деплоя выполняются на целевом сервере: CI/CD или вручную по SSH
- Локальная проверка скриптов - статическая (bash -n, shellcheck, разбор логики); полный прогон - на тестовом хосте
...
- Новые записи следуют стилю уже существующих записей CHANGELOG.md: без усложнения формулировок и лишних подробностей
- Не добавлять в CHANGELOG ссылки на задачи (номера вида #N)
- Описания изменений должны быть емкими: что изменилось, без причинно-следственных объяснений и деталей реализации
...
- Не использовать длинное тире (U+2014): вместо него пишется дефис "-"
- Не использовать стрелку (U+2192): вместо неё пишется "->"
- Правило действует во всех текстовых файлах проекта: документации, комментариях, docstring и строках кода
...
- Проверять Markdown файлы с помощью markdownlint: `npx markdownlint-cli '**/*.md' --ignore-path .gitignore`
...
- Документировать только актуальное состояние: история изменений
  и причин принятых решений живет в коммитах и CHANGELOG.md,
  а не в документации
- Глаголы в документации использовать в неопределенной форме,
  а не в повелительном наклонении («удалить», а не «удалите»)
- Писать документацию в docs/ связной прозой для людей: полные
  предложения со сказуемыми, связный текст разделов, объяснения
  «почему» и «что из этого следует»; рубленый телеграфный стиль без
  глаголов уместен только в скиллах (.agents/skills/), списках команд
  и CHANGELOG.md

## Особенности развертывания

- Используется Gunicorn как WSGI сервер
- nginx для проксирования, HTTPS и раздачи медиа; сертификаты выпускает и продлевает certbot
