---
name: server-setup
description: Первоначальная настройка нового сервера под развертывание проекта (Docker Compose или systemd Gunicorn) или миграция на новый сервер. Скилл дополняет scripts/docker/setup.sh и scripts/server/setup.sh знанием об окружении, которое скрипты не учитывают.
---
