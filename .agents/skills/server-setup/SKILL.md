...
Сервером может быть виртуальная машина облачного провайдера или
физический сервер, однопроектный или с несколькими проектами.
Провайдер-специфичных шагов скилл не содержит: параметры сервера
берутся из консоли провайдера или от администратора железа.
...
- nginx в compose (профиль nginx) - вариант однопроектного сервера:
  официальный образ, шаблон nginx/personal-website.conf.template,
...
- Хостовый nginx + certbot - вариант сервера с несколькими проектами
  (и однопроектного без Docker): конфигурация сайта на проект
...
- COMPOSE_PROFILES по выбранным вариантам: пусто при внешних БД и S3
  с прокси на хосте; добавить nginx, если HTTPS обслуживает
...
Вариант systemd-режима с unix-сокетом (socket activation) - там же,
[application.md](../../../docs/deployment/application.md),
раздел 4.
...
- Прокси-слой проекта не занимает 80/443: профиль nginx не
  активируется, HTTPS обслуживает общий хостовый nginx по рецепту
