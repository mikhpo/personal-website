...
| Облачный сервер | nginx | nginx+certbot в compose | managed | S3 удаленный |
| Домашний сервер | postgres,minio | хостовый nginx+certbot | контейнер | MinIO (s3) |
...
- Хостовый nginx + certbot - основной путь, в том числе для
  мультипроектного домашнего хоста: конфигурация сайта на проект
