...
Вывод `src pr checks` содержит строку `STATUS:` (верхний уровень - `success` или `failure`), а также строки по каждому workflow (`success` / `failed`). Извлекать статус нужно из первой строки `STATUS:`, беря последнее слово через `awk`, и сравнивать через `case` - это надёжнее grep-паттернов с `\s`, которые не срабатывают на macOS (BSD grep) и при наличии ANSI-кодов.
