...
def verify_command():
    ...
    '''
    answer = input('Внимание! Выполнение данного скрипта может привести к потере данных!\nВы уверены, что желаете продолжить выполнение скрипта? [y/n]')
    if answer.lower() == 'n':
...
def check_dump(dump_path: str):
    ...
    '''
    if not dump_path:
        dump_path = input('Не указан адрес дампа. Пожалуйста, укажите полный адрес дампа: ')
        if not dump_path:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Восстановление базы данных PostgreSQL из дампа')
