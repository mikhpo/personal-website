...
# Каталоги переводов проекта: строки сторонних пакетов без русской локали
# (например, django-auditlog) локализуются здесь. Скомпилированные .mo
# в репозиторий не попадают - каталог компилируется при сборке образа.
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            ...
            "libraries": {
                ...
                "react_tags": "personal_website.templatetags.react_tags",
            },
        },
    },
]
...
# Размеры в пикселах по наибольшей стороне для постоянных ссылок
# на превью фотографий, вставляемых в статьи и сторонние ресурсы.
# Набор расширяется добавлением значений в кортеж: варианты появляются
# в селекторе на странице фотографии и в админке после перезапуска,
# миграции и перегенерации существующих превью не требуются. Порядок
# значений определяет порядок в интерфейсе. Размер меньше исходника
# не увеличивается: для оригинала 500px вариант 640 вернет 500px.
GALLERY_EMBED_SIZES = (400, 800, 1200, 1600)
