...
function main() {
    ...
    # и запускается в обоих режимах: web-процессы ставят задачи
    # в очередь, воркер их потребляет - порядок старта контейнеров
    # не имеет значения.
    $python "$manage" migrate
    $python "$manage" create_huey_tables

    # Воркер потребляет очередь huey и не обслуживает HTTP:
    # статические файлы ему не нужны. exec замещает оболочку процессом
    # воркера, чтобы сигналы остановки контейнера доходили до него.
