...
Каждый инфраструктурный сервис относится к собственному профилю: postgres, minio, traefik. Явное указание имени сервиса в команде активирует профиль автоматически, поэтому флаг для запуска отдельных сервисов не требуется. Для типичной локальной разработки достаточно профиля postgres с файловым хранилищем: `docker compose up -d postgres` или переменная `COMPOSE_PROFILES='postgres'` в файле `.env`. Профиль minio подключается при разработке с S3-хранилищем, traefik - при отладке прокси-слоя. Без активных профилей поднимается только приложение (сервис application) - этот режим подходит для окружений, где используются внешний PostgreSQL, внешнее объектное хранилище и хостовый обратный прокси.
...
В разработке переменная `TRAEFIK_CERT_RESOLVER` остаётся пустой: ACME-клиент пассивен, HTTPS обслуживается встроенным self-signed сертификатом Traefik. Проверка маршрутизации выполняется локально без влияния на домен:

    # Маршрутизация по HTTPS (флаг -k принимает self-signed сертификат):
    curl -k --resolve <домен>:443:127.0.0.1 https://<домен>/

    # Редирект HTTP -> HTTPS:
    curl -sI --resolve <домен>:80:127.0.0.1 http://<домен>/ | head -3

Признаки того, что выпуск сертификатов не выполнялся: файл `traefik/letsencrypt/acme.json` отсутствует или пуст; вывод команды `docker compose logs traefik | grep -i acme` пуст.
...
Прокси-сервер Traefik терминирует HTTPS: сертификаты Let's Encrypt получаются и обновляются встроенным ACME-клиентом автоматически, конфигурация маршрутизации задаётся декларативно лейблами сервисов в [compose.yaml](../compose.yaml). Настройка сертификатов, включая перенос между серверами, описана в [docs/deployment/proxy.md](./deployment/proxy.md); проверка работы в среде разработки - в разделе «Контейнеризация» выше.
