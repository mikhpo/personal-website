...
Сервисы postgres и minio относятся к профилю dev: без активного профиля поднимаются только website и nginx. Такой режим предназначен для окружений, где используется внешний PostgreSQL и внешнее объектное хранилище.
