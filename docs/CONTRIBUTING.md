...
Механика без файлов с ключами и конфигами: ключ SYNC_GITHUB_SSH_KEY добавляется в ssh-agent через stdin, проверка хостов - StrictHostKeyChecking=accept-new; перед запуском куб выполняет `git fetch --unshallow` на случай поверхностной автозагрузки кода. Сбой синхронизации не прерывает работу конвейера - в лог куба пишется предупреждение.
...
Запускает скрипт `tools/sync_mirrors.py` (задача `sync-repositories`): выравнивание основной ветки и тегов между зеркалами. Ключ SYNC_SOURCECRAFT_SSH_KEY добавляется в ssh-agent через stdin, проверка хостов - StrictHostKeyChecking=accept-new; checkout выполняется с fetch-depth: 0 для полной истории. Сбой синхронизации фиксируется предупреждением и не прерывает конвейер.
