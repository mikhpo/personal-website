...
Прокси-сервер Traefik терминирует HTTPS: сертификаты Let's Encrypt получаются и обновляются встроенным ACME-клиентом автоматически, конфигурация маршрутизации задаётся декларативно лейблами сервисов в [compose.yaml](../compose.yaml). Настройка сертификатов, включая перенос между серверами, описана в разделе [«Сертификаты Let's Encrypt (Traefik)»](./README.md#сертификаты-lets-encrypt-traefik) файла README.md; проверка работы в среде разработки - в разделе «Контейнеризация» выше.
