...
- backups.md: раздел о смене типа хранилища дополнен шагом удаления статики из временной цели при обратном переносе из s3 (медиа-бакет содержит префикс static/), раздел об алиасах - порядком настройки в Docker Compose
