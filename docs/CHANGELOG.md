# История изменений

## 2026-08-26

- Из шаблонов конфигурационных файлов (systemd-юнит, vhost nginx, bootstrap-vhost ACME) удалены комментарии: логика описывается в документации; в docs/deployment/application.md и proxy.md добавлено недостающее (префикс ":" в ExecStart, ответ 503 bootstrap-vhost), образцы в документации приведены к фактическому содержимому шаблонов
- Термин «ось» изъят из документации по решению пользователя: пять независимых параметров развертывания (способ запуска приложения, размещение базы данных, тип хранилища, вариант прокси, конфигурация резервного копирования) называются естественными словами без сквозного собирательного термина; «эталонные конфигурации» переименованы в «эталонные сценарии развертывания» (A-D); фразы переформулированы целиком
- Созданы рецепты развертывания docs/deployment/proxy.md (хостовый nginx + certbot: vhost, двухфазный выпуск сертификата webroot, systemd timer и deploy-hook, вариант DNS-01, режим Traefik в compose, пост-мортем nginx-в-контейнере, неподдерживаемые варианты) и docs/deployment/application.md (контракт приложения, Docker Compose, полная команда docker run с сетями и bind-монтами, systemd Gunicorn с юнитом и логами journal)
- Переменная TRAEFIK_ACME_CASERVER в compose.yaml и .env.example: адрес ACME CA-сервера Traefik - пусто для продакшена Let's Encrypt, staging-каталог для тестовых сред (защита от исчерпания rate limits)
- Скрипты scripts/server/ переработаны под текущую архитектуру: приложение слушает 127.0.0.1:${DJANGO_PORT} (unix-сокет Gunicorn удален), юнит personal-website.service с ExecStartPre миграций/collectstatic и формулой воркеров entrypoint.sh, vhost nginx из шаблона (proxy_pass на 127.0.0.1, X-Forwarded-заголовки, client_max_body_size 50m), выпуск сертификата certbot certonly webroot с ACME staging для тестовых сред; удалены устаревшие makelink.sh и шаблоны backend/config/
- Скилл server-setup дополнен матрицей профилей compose, двумя режимами прокси (traefik в compose / хостовый nginx + certbot), порядком настройки systemd-варианта и сценарием homelab; устаревшие упоминания старой схемы убраны
- Исправлена compose-ветка scripts/backup.sh: команды приложения запускались системным интерпретатором Python образа, в котором не установлен Django; используется интерпретатор виртуального окружения приложения (найдено функциональным прогоном на dev-стеке)

## 2026-08-25

- Механизм staging-каталога исключен из резервного копирования медиа: при STORAGE_TYPE=s3 синхронизация и восстановление выполняются напрямую (mc адресует медиа-бакет алиасом, rclone - remote, собираемым из переменных RCLONE_CONFIG_* приложения, rclone.conf не требуется); прежде перенос шел через промежуточную локальную копию в TEMP_ROOT, удваивавшую требования к диску
- Восстановление и проверка (verify) дампов БД выполняются конвейером mc cat / rclone cat | pg_restore без выгрузки дампа на диск; приложение backup не использует TEMP_ROOT
- Механизмы копирования выделены в плагины-цели: классы McTarget и RcloneTarget реализуют общий интерфейс Target и регистрируются в реестре TARGET_PREFIXES, новый механизм подключается подклассом и записью в реестр без изменений команд и оркестрации; диспетчеризация по префиксу цели убрана из функций services.py
- Приложение backup декомпозировано на модули process.py (запуск внешних команд и конвейеров), targets.py (плагины-цели), utils.py (адресация медиа-хранилища и вспомогательные функции) и services.py (оркестрация операций)
- MC_PATH требуется только при наличии mc-целей: при filesystem-хранилище и rclone-целях MinIO Client не нужен
- Докстринги модулей приложения backup дополнены секциями Args/Returns/Raises/Example и описаниями атрибутов классов

## 2026-08-22

- Система резервного копирования реализована Django-приложением backend/backup/ с management-командами backup, backup_db, backup_media (с флагом --verify для проверки целей) и restore_db, restore_media; хостовый вход - scripts/backup.sh, запускающий команды по расписанию cron
- Резервное копирование БД: дамп pg_dump -Fc с проверкой pg_restore -l, копирование в цели BACKUP_DB_TARGETS, ретеншн BACKUP_DB_RETENTION_DAYS в целях и локально, verify (свежесть дампов и читаемость последнего); восстановление pg_restore в существующую базу из дампа цели или локального файла
- Резервное копирование медиа: зеркальная синхронизация в цели BACKUP_MEDIA_TARGETS и восстановление из цели, verify (наличие storage/ и число объектов против источника)
- Цели бэкапов описываются в .env записями BACKUP_TARGET_<ИМЯ> с префиксами mc: (MinIO Client) и rclone: (rclone); rclone добавлен в Docker-образ приложения, смена типа хранилища выполняется через временную цель
- Задача cron резервного копирования переведена на scripts/backup.sh backup, журнал - logs/backup.log; диспетчер деплоя scripts/deploy.sh выполняет предварительное резервное копирование вызовом scripts/backup.sh backup
- Добавлены тесты приложения backup на базе django.test, покрывающие операции модулей

## 2026-08-20

- В AGENTS.md зафиксировано правило: скрипты деплоя не запускаются на локальной машине разработчика (переключение рабочей копии на main, остановка стека, удаление неиспользуемых образов Docker всей машины); локальная проверка скриптов - статическая
- Добавлена проверка shellcheck в pre-commit (hook shellcheck-py); предупреждения исправляются, а не подавляются: в tools/pg-isready.sh шебанг перенесен на первую строку и закавычены параметрические экспансии (SC1128, SC2086); исключение для source рантайм-файлов (.env, /etc/os-release) объявлено централизованно в .shellcheckrc (SC1090, SC1091), построчные директивы disable из s3backup.sh и s3restore.sh удалены
- Добавлен общий диспетчер деплоя scripts/deploy.sh - единый вход для CI/CD и ручного обновления: выполняет резервное копирование медиафайлов (s3backup.sh) и вызывает скрипт механизма развертывания по переменной DEPLOY_MODE (compose - scripts/docker/deploy.sh, systemd - scripts/server/deploy.sh; пусто или отсутствие переменной - как compose, неизвестное значение - ошибка со списком допустимых); деплой на SourceCraft и GitHub переведен с scripts/docker/deploy.sh на диспетчер
- Отсутствие файла .env - фатальная ошибка в s3backup.sh и s3restore.sh (унификация с диспетчером деплоя): все обязательные переменные читаются из него, продолжение без него приводило бы к позднему падению на проверках переменных
- Скрипт scripts/docker/update.sh удален как дублирующий scripts/docker/deploy.sh: резервное копирование перед деплоем выполняет диспетчер; очистка неиспользуемых образов перенесена в deploy.sh и ограничена образами проекта фильтром по метке org.opencontainers.image.title, добавленной в Dockerfile (глобальный prune затрагивал бы образы других проектов хоста)
- Переменная DEPLOY_MODE описана в .env.example
- Скрипты s3backup.sh и s3restore.sh поддерживают перенос медиафайлов между бэкендами хранения при смене STORAGE_TYPE: первым аргументом задается приемник (s3backup.sh) или источник (s3restore.sh) вида alias/бакет[/префикс], по умолчанию - бакет бэкапов; адрес без алиаса отвергается; остальные аргументы передаются mc mirror напрямую (например, --dry-run)
- Загрузка .env в скриптах s3backup.sh и s3restore.sh переведена с eval export на безопасную (set -a + source с проверкой существования файла); в s3restore.sh исправлен путь к .env: прежде он вычислялся внутри каталога scripts/, файл не находился и переменные окружения не загружались
- Ограничение скорости --limit-download убрано из зашитых флагов s3restore.sh: передается аргументом mc mirror при необходимости
- Добавлен блок безопасности для развертывания за обратным прокси с терминацией HTTPS: настройки SECURE_SSL_REDIRECT, SECURE_HSTS_SECONDS, SESSION_COOKIE_SECURE, CSRF_COOKIE_SECURE и SECURE_PROXY_SSL_HEADER задаются переменными окружения с безопасными для разработки дефолтами (все механизмы выключены); переменные описаны в .env.example
- CSRF_TRUSTED_ORIGINS собирается автоматически из DOMAIN_NAME (адреса `https://домен` и `https://www.домен`); прежде настройка отсутствовала
- Записи домена в ALLOWED_HOSTS и CORS_ALLOWED_ORIGINS добавляются только при заданном DOMAIN_NAME: прежде при незаданной переменной в списки попадали значения `None` и `www.None`
- В .env.example все переменные показаны раскомментированными со значениями по умолчанию: опциональные (`POSTGRES_SSL_*`, `GUNICORN_WORKERS`) - с пустым значением или дефолтом, конкретные значения упоминаются в комментариях
- Добавлен раздел о типографике в AGENTS.md и pre-commit хук fix-typography (tools/fix_typography.py): длинное тире U+2014 заменяется на дефис, стрелка U+2192 - на "->"; существующие вхождения заменены во всех файлах репозитория, включая строку навигации галереи и её тест

## 2026-08-19

- Сервис приложения в compose.yaml переименован из website в application (имя контейнера personal-website-application-1); traefik-лейблы маршрутизации сохранены без изменений
- Инфраструктурные сервисы переведены с единого профиля dev на собственные профили postgres, minio и traefik: каждый включается независимо; дефолт .env.example для локальной разработки - только postgres (minio подключается при разработке с S3, traefik при отладке прокси-слоя), пустое значение COMPOSE_PROFILES означает полностью внешнюю инфраструктуру (поднимается только приложение)
- Порт приложения постоянно публикуется на 127.0.0.1:${DJANGO_PORT} для хостового обратного прокси; порты postgres и MinIO перевязаны на 127.0.0.1
- Сервису application добавлена запись extra_hosts host.docker.internal:host-gateway для доступа к systemd-сервисам хоста с Linux
- Healthcheck контейнера приложения переключен с корневого URL на новый эндпоинт /health/, выполняющий проверочный запрос SELECT 1 к базе данных (200 при доступной БД, 503 при ошибке)
- Ожидание готовности объектного хранилища в entrypoint.sh выполняется только при STORAGE_TYPE=s3 и проверяет эндпоинт AWS_S3_ENDPOINT_URL на уровне TCP (провайдер-независимо: локальный MinIO и публичное облако проверяются одинаково); при STORAGE_TYPE=filesystem приложение от S3 не зависит и ожидание не выполняется - прежде при s3 с внешним хранилищем каждый старт ожидал таймаут несуществующего локального MinIO (127.0.0.1:9000)
- Настройка алиаса MinIO в контейнере (set_minio_alias в entrypoint.sh) удалена: потребителей алиаса внутри контейнера нет, резервное копирование выполняется на хосте собственным клиентом mc
- Число воркеров Gunicorn может задаваться переменной окружения GUNICORN_WORKERS (перекрывает формулу 2×CPU+1 для окружений с ограниченной оперативной памятью)
- Задача Taskfile test-integration поднимает сервис приложения и дожидается прохождения healthcheck (--wait) перед запуском тестов; интеграционные тесты robots.txt и favicon.ico принимают оба режима раздачи статики (WhiteNoise напрямую при filesystem, редирект при s3)
- Документация (README, CONTRIBUTING, AGENTS) и .env.example синхронизированы с новыми именами сервисов и профилями

## 2026-08-17

- Имя образа приложения в compose.yaml унифицировано с публикуемым SourceCraft CI (mikhpo/personal_website, подчёркивание): прежде compose ссылался на несуществующий образ с дефисом, и docker compose pull падал ошибкой object not found
- entrypoint.sh: настройка алиаса MinIO пропускается при STORAGE_TYPE=s3 - команда mc admin info неприменима к внешнему S3-хранилищу и уводила контейнер в crash-loop
- entrypoint.sh: заменён eval export файла .env на безопасную загрузку с проверкой существования - при отсутствии файла export без аргументов печатал значения секретных переменных в логи контейнера
- Инструкции развертывания (README, CONTRIBUTING) дополнены адресом клонирования SourceCraft как основным; задокументировано экранирование символа доллара в значениях .env для Docker Compose ($$, раздел CONTRIBUTING «Экранирование символа доллара»): без удвоения compose молча искажает значения вроде SECRET_KEY
- deploy-workflow SourceCraft переключён на целевой каталог /srv/personal-website

## 2026-08-16

- Gunicorn в контейнере запускается с рециклингом воркеров (--max-requests=500, --max-requests-jitter=50): после обработки фотографий (Pillow, ImageKit) воркер удерживает память и не возвращает её аллокатору, перезапуск воркера по счётчику запросов ограничивает дрейф RSS (замер: рост воркера со ~110 до ~260 МБ после пиковой загрузки фото)

## 2026-08-15

- Прокси-сервер nginx заменён на Traefik (v3.7): обслуживает HTTPS с автоматическими сертификатами Let's Encrypt, запросы маршрутизируются по Docker-лейблам; каталог nginx/ и том nginx-certs удалены
- Состояние ACME-клиента Traefik хранится на хосте (bind-mount traefik/letsencrypt/acme.json): команда docker compose down -v сертификаты не затрагивает; каталог добавлен в .gitignore как содержащий приватные ключи
- Выпуск сертификатов управляется переменной TRAEFIK_CERT_RESOLVER: в продакшене le (Let's Encrypt), в разработке пустая строка (self-signed, ACME-клиент пассивен)
- Публикация порта приложения 8000 наружу прекращена: весь трафик проходит через Traefik (80/443)
- Логирование контейнеров переведено на комбинированную стратегию: якорь x-logging (json-file, ротация 10 МБ x 3 файла) применён ко всем сервисам; журналы доступа и ошибок Gunicorn пишутся в stdout
- Логгер django.request единообразно подключён к консольному хэндлеру наряду с остальными логгерами (схема хэндлера console не менялась)
- Скрипт scripts/docker/deploy.sh перепрофилирован под неинтерактивный деплой из CI/CD (обновление репозитория, подготовка каталогов, пересоздание контейнеров); первичная настройка сервера вынесена в новый скрипт scripts/docker/setup.sh (пакеты, Docker, ufw, cron бэкапов)
- Из скрипта обновления bare-metal-развертывания убран вызов pgcert.sh: получение SSL-сертификата PostgreSQL выполняется при первичной настройке (scripts/server/setup.sh, scripts/docker/setup.sh) и вручную при ротации CA
- Скрипты bare-metal-развертывания переименованы в соответствие с Docker-скриптами: scripts/server/deploy.sh (первичная настройка) -> setup.sh, scripts/server/update.sh (деплой обновлений) -> deploy.sh
- deploy-workflow SourceCraft и зеркальный GitHub workflow переключены на scripts/docker/deploy.sh
- Из Dockerfile убраны неиспользуемые пакет cron и вызов scripts/cronjobs.sh: задачи cron выполняются на хосте через setup.sh
- Порт приложения вынесен в переменную окружения DJANGO_PORT (используется Traefik и entrypoint.sh); переменная NGINX_PORT переименована в TRAEFIK_HTTP_PORT (compose.yaml, tests, .env.example)
- Интеграционные тесты прокси адаптированы под Traefik (tests/test_traefik.py): HTTPS с self-signed сертификатом, проверка редиректа HTTP на HTTPS
- Документация (README, AGENTS) дополнена разделами о Docker-развертывании, сертификатах Let's Encrypt и проверке Traefik в среде разработки; скилл production-diagnostics переписан под Docker-окружение
- C4-диаграмма контейнеров дополнена компонентами Traefik, Docker Engine, Docker Compose и реестрами образов

## 2026-08-14

- Сервисы postgres и minio в compose.yaml переведены на профиль dev: без активного профиля поднимаются только website и nginx (режим для окружений с внешним PostgreSQL и внешним объектным хранилищем), профиль активируется переменной COMPOSE_PROFILES=dev или флагом --profile dev
- Зависимости depends_on сервиса website от postgres и minio заменены ожиданием готовности сервисов в entrypoint.sh (функция wait_for_port с ограничением числа попыток)
- В .env.example добавлена переменная COMPOSE_PROFILES для активации dev-профиля
- Задачи Taskfile up-detached, watch и restart обновлены под профиль dev; флаг --wait подставляется условно и применяется только при наличии Docker Compose V2
- Документация (README, CONTRIBUTING, AGENTS) дополнена описанием профилей Docker Compose
- Раздача статических файлов переведена на объектное хранилище: в S3-режиме collectstatic собирает файлы в префикс static/ того же бакета (хранилище выбирается селектором select_static_storage), локально и в тестах продолжает работать WhiteNoise
- Заголовки кэширования статических файлов в S3: сборкам с хешем содержимого в имени назначается бессрочное immutable-кэширование, остальным файлам - умеренный срок
- STATIC_URL и webpack publicPath вынесены в переменные окружения (STATIC_URL, WEBPACK_PUBLIC_PATH); update.sh загружает .env до сборки фронтенда
- Запросы /favicon.ico и /robots.txt обслуживаются постоянными редиректами Django на адрес статического хранилища вместо раздачи nginx из локальной файловой системы
- WhiteNoise (middleware и WHITENOISE_ROOT) отключается в S3-режиме
- Из конфигураций nginx (bare-metal и Docker) удалены файловые локации /static/, /media/, /favicon.ico, /robots.txt: nginx выполняет только проксирование, статические и медиафайлы обслуживает объектное хранилище
- Интеграционные тесты nginx обновлены под редиректы favicon и robots.txt
- Каталог node_modules исключён из STATICFILES_DIRS: tinymce и bootstrap/dist копируются сборкой фронтенда в backend/staticfiles, объём выгрузки статики сокращается с десятков тысяч файлов до нескольких сотен
- Из продакшен-выгрузки статики исключены source-map'ы (*.map) и документация (*.md)
- Таймаут удалённой команды деплоя увеличен до 30 минут в конфигурациях SourceCraft и GitHub

## 2026-08-13

- Самописные скрипты бэкапа и восстановления БД (scripts/pgbackup.sh, scripts/pgrestore.sh) заменены стандартными утилитами pg_dump и pg_restore; процедуры перенесены в документацию (раздел README) и скиллы (postgresql-dump, postgresql-restore)
- Из scripts/server/restart.sh убран перезапуск базы данных: сервер приложения не отвечает за управление СУБД
- Из scripts/cronjobs.sh и scripts/docker/update.sh удалены вызовы убранных скриптов

## 2026-08-11

- SSL-сертификат для Managed PostgreSQL доставляется в контейнер при запуске, а не при сборке образа
- Добавлен скрипт scripts/pgcert.sh для загрузки SSL-сертификата PostgreSQL (вызывается из deploy.sh и update.sh)
- Из Dockerfile и compose.yaml удалены build-args; в compose.yaml добавлен bind-mount сертификата через POSTGRES_SSL_ROOT_CERT_PATH (контейнер) и опциональную POSTGRES_SSL_CERT_HOST_PATH (хост)
- Раздел по настройке базы данных перенесён из CONTRIBUTING.md в README.md
- Унифицирована логика публичности объектов с полем public (Album, Photo, Article, Category, Topic, Series): списки и sitemap показывают только public=True, детальный просмотр любого объекта по прямой ссылке доступен всем
- Исправлена ошибка 404 при доступе к скрытым фотографиям по прямой ссылке, приходящей со страниц альбомов
- Удалены staff-ветки в get_queryset просмотровых views и viewsets: staff и не-staff видят основной сайт одинаково, для администрирования служит отдельный интерфейс /admin/
- Запись объектов с полем public (создание/изменение/удаление через REST API) ограничена администраторами; раннее разрешение «любому аутентифицированному» заменено на staff-only. Комментарии к статьям по-прежнему может оставлять любой аутентифицированный пользователь, а изменять/удалять - только автору комментария или администратору
- Вложенные связи в retrieve (фотографии внутри альбома) фильтруются по public=True
- Prev/next навигация на детальной странице фотографии теперь строится только по публичным соседям альбома
- На детальные страницы скрытых объектов добавлен meta robots noindex для запрета индексации share-ссылок поисковиками
- Расширены тесты API и views под новую инварианту публичности

## 2026-08-09

- Подготовлена миграция базы данных на Managed Service for PostgreSQL
- Добавлена документация по развертыванию БД в CONTRIBUTING.md
- Версия PostgreSQL клиента: 15 -> 17

## 2026-07-30

- Добавлено поле exif (JSONField) в модель Photo для хранения EXIF данных в БД вместо чтения из файла при каждом запросе
- Добавлена миграция данных для заполнения поля exif существующих фотографий
- Добавлена функция exif_value_to_json в gallery/utils.py для преобразования значений EXIF в JSON-совместимые типы
- Добавлены тесты для проверки сохранения и чтения EXIF данных из БД

## 2026-07-26

- Исправлена ошибка 500 при browsable-доступе к API: приложение django_filters добавлено в INSTALLED_APPS
- Исправлены пустые списки на страницах тега и категорий блога: генерация URL фильтрации перенесена во фронтенд

## 2026-07-24

- Реорганизована структура проекта в монорепозиторную архитектуру с разделением на backend и frontend
- Реализован REST API с JWT аутентификацией, кастомной пагинацией и правами доступа
- Добавлены сериализаторы и viewsets для приложений blog и gallery с полным CRUD функционалом
- Создано централизованное приложение api для управления REST API
- Мигрирован frontend на React с компонентной архитектурой
- Реализованы React компоненты для всех основных разделов: блог, галерея, навигация, пагинация
- Добавлены кастомные React хуки для работы с API данными
- Интегрирован Webpack с Babel для сборки React приложений
- Настроен процесс тестирования React компонентов с Jest
- Добавлены компоненты для загрузки фотографий с drag-and-drop и прогрессом загрузки
- Обновлён редактор WYSIWYG на TinyMCE
- Настроена интеграция React компонентов через Django template теги с автоматическим монтированием
- Добавлены команды управления для генерации тестовых объектов блога и галереи
- Обновлены CI/CD пайплайны для поддержки новой структуры проекта
- Добавлены навыки (skills) для статического анализа кода и обновления CHANGELOG

## 2025-12-25

- Добавлена проверка переменной окружения STORAGE_TYPE в скрипт резервного копирования S3 для предотвращения ненужных операций резервного копирования при использовании S3 в качестве хранилища

## 2025-12-24

- Реализована поддержка S3-совместимого хранилища (Amazon S3 или MinIO) в дополнение к локальной файловой системе
- Добавлена возможность переключения между локальной файловой системой и S3-хранилищем через настройку STORAGE_TYPE
- Созданы кастомные классы хранилищ CustomFileSystemStorage и CustomS3Storage с расширенным функционалом
- Обновлены тесты галереи и утилиты для работы через абстрактный storage API вместо прямых обращений к файловой системе

## 2025-11-28

- Добавлена конфигурация CI/CD для SourceCraft

## 2025-11-15

- Добавлена стадия деплоя на VPS
- Оптимизирована сборка образа в Dockerfile

## 2025-08-21

- Исправлена проблема определения времени выдержки в приложении галереи, приводившая к ошибке 500.
- Исправлена проблема при генерации значений объектов фабриками в тестах, когда нарушалось ограничение уникальности полей.

## 2025-08-10

- Добавлены джобы пайлайна для тестирования контейнеризованного приложения.
- Вместо сборки образа при релизе выполняется перетегировани протестированного образа.

## 2025-08-08

- Улучшена документация на проект.
- Добавлены архитектурные диаграммы.

## 2025-02-01

- Доработан механизм автоматического конфигурирования Gunicorn.

## 2025-01-06

- Внедрена система проверки типов для Python.

## 2024-12-23

- Генерация изображений и данных EXIF для тестов.

## 2024-10-26

- Добавлен скрипт для генерации диаграмм моделей.

## 2024-10-19

- Добавлен тэг шаблона для проверки наличия файлов.
- Увеличина ширина виджета TinyMCE.
- Обновлены версии зависимостей.

## 2024-10-13

- Доработка вспомогательных скриптов и команд.

## 2024-04-25

- Исправление ошибки выполнения команд MinIO Client через cron.

## 2024-04-20

- Дополнения документации в части развертывания.
- Исправления скриптов для развертывания.

## 2024-02-04

- Добавлен статический анализ кода при помощи Ruff.
- Форматирование кода для соответствия правилам Ruff.
- Добавлен статический анализ кода при помощи ESLint.

## 2024-01-28

- Добавлены скрипты для синхронизации бэкапов с S3.

## 2024-01-20

- Добавлены шаблоны задач и запросов на извлечения для GitHub.
- Рефакторинг скрипта для подготовки коммита.
- Добавлен Taskfile.yml для Task.

## 2024-01-15

- Рефакторинг скриптов для работы с бэкапами.
- Добавлены скрипты для развертывания через Docker.
- Добавлены хуки в Git.

## 2024-01-09

- Добавлены CI/CD для тестирования кода и публикации образов.

## 2024-01-06

- Nginx и Certbot добавлены в Docker Compose.

## 2024-01-01

- Приложение запускается в контейнере через Gunicorn.

## 2023-12-25

- Подготовлены инструкции по сборке и запуску контейнеров.
- Переименование каталога с модулем настроек Django.

## 2023-11-12

- Изменена сортировка фотографий в представлении списка фотографий и добавлена паджинация.
- Добавлено создание отчета Coverage по тестированию.

## 2023-11-06

- Альбомы без обложек не показываются в списке альбомов.
- Оптимизировано представление тэгов галереи.

## 2023-11-05

- Изменена структура каталогов для хранения изображений галереи.

## 2023-11-02

- Добавлены сообщения о результатах загрузки фотографий в альбом.

## 2023-10-22

- Отображение данных из EXIF в административной панели.
- Оптимизация представления карточек Bootstrap.
- Обновление версий зависимостей.

## 2023-10-21

- Добавлены кнопки для переключения между фотографиями внутри альбома.

## 2023-10-19

- Данные EXIF фотографии показываются в модальном окне.

## 2023-10-08

- Добавлен скрипт для форматирования кода и отправки коммита в удаленный репозиторий.
- Добавлена форма для пакетной загрузки фотографий в альбом.
- Добавлен подсчет количества фотографий в альбоме.

## 2023-09-28

- Настроен Pytest для прогона тестов.

## 2023-09-26

- Исправлен скрипт для обновления проекта.

## 2023-09-23

- Добавлены миниатюры и предпросмотр фотографий и обложек альбомов в административную панель.
- Добавлен показ списка фотографий при просмотре альбома в административной панели.
- Исправлен показ только публичных фотографий и альбомов в пользовательских представлениях.

## 2023-09-18

- Фотографии в административной панели сортируются от старых к новым.
- Добавлены конфигурации nginx и gunicorn.

## 2023-09-03

- Создано приложение галереи.

## 2023-01-07

- Добавлен скрипт для бэкапирования хранилища.

## 2022-12-24

- Добавлены тесты системы авторизации.

## 2022-12-22

- Добавлены тесты блога и главной страницы.
- Исправление стилей таблиц.

## 2022-11-25

- Рефакторинг структуры проекта.
- Удалено приложение для управления скриптами.
- Управление секретами через переменные окружения и .env файл.

## 2022-04-26

- Добавлены вспомогательные скрипты для управления проектом.

## 2022-01-26

- Оптимизация приложения для управления скриптами.

## 2022-01-17

- Исправления шаблонов.

## 2022-01-10

- Рефакторинг структуры проекта.

## 2022-01-08

- Доработка планировщика скриптов.

## 2021-080-07

- Оптимизация моделей и маршрутов блога.

## 2021-08-05

- Добавлена карта сайта.

## 2021-08-04

- Доработка приложения для управления скриптами.

## 2021-07-18

- Рефакторинг управления зависимостями через Poetry.
- Уравление секретами через JSON файл.

## 2021-07-05

- Добавлено приложение для управления скриптами.

## 2021-07-03

- Обновление зависимостей проекта
- Доработка механизма регистрации пользователей.

## 2021-02-22

- Первоначальный релиз.

## 2020-12-19

- Старт разработки.
