...
- pipe_commands считает штатным завершением источника конвейера по SIGPIPE при успешном получателе: pg_restore -l читает только заголовок и оглавление дампа и не дочитывает поток mc cat / rclone cat, из-за чего проверка --verify дампов больше пары килобайт падала с ошибкой «дамп не читается» (#109)
...
- entrypoint.sh настраивает алиас mc при старте контейнера из переменных MINIO_ALIAS, MINIO_SERVER_URL, MINIO_ACCESS_KEY и MINIO_SECRET_KEY: команды резервного копирования выполняются внутри контейнера и без алиаса теряли доступ к mc-целям после каждого пересоздания (#103)
...
- Смена типа хранилища выполняется через постоянные цели бэкапов: backup_media на старом хранилище, restore_media на новом; временные цели и правки .env на время переноса не применяются (#104)
- create_data_directories в setup.sh передает каталоги данных сервис-пользователю рекурсивно: файлы, оставшиеся от контейнерного запуска, блокировали старт systemd-сервиса; порядок перехода описан в application.md (#105)
