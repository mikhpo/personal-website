...
- Инфраструктурные сервисы переведены с единого профиля dev на собственные профили postgres, minio и traefik: каждый включается независимо, пустое значение COMOSE_PROFILES означает полностью внешнюю инфраструктуру (поднимается только приложение)
