...
- Traefik заменен на nginx + certbot в Docker Compose: официальный образ nginx (профиль nginx) с шаблоном конфигурации nginx/personal-website.conf.template, хранилище монтируется в прокси для раздачи медиа напрямую с диска, certbot - отдельный контейнер (первичный выпуск standalone до старта nginx в scripts/docker/setup.sh, продление renew --webroot по cron хоста через scripts/docker/renew-cert.sh; альтернативы - sleep-loop контейнер и ручной УЦ - в docs/deployment/proxy.md); self-signed сертификат локального стека - задача task dev-cert; переменные TRAEFIK_* удалены, добавлены HTTP_PORT/HTTPS_PORT
- Медиа в filesystem-режиме раздаются прокси напрямую с диска, минуя приложение: location /media/ с alias на STORAGE_ROOT в шаблоне хостового nginx (setup.sh и deploy.sh рендерят путь) и в конфигурации контейнерного nginx; интеграционный тест раздачи медиа через прокси
- Типовые сценарии развертывания в matrix.md описывают только продакшен: «Облачный сервер», «Домашний сервер», «Сервер без Docker» (были «облако-соло», «homelab», «bare-metal»); локальная разработка выведена из сценариев, ее порядок - в README
...
- Добавлены скиллы backup-restore (операции бэкапа/восстановления, смена типа хранилища), backup-audit (проверка настройки) и restore-drill (учебное восстановление на тестовом окружении, только по явному запросу пользователя)
- Проверка --verify медиа-целей сообщает о любом расхождении числа объектов с источником (ранее порог 5%): зеркало полное, расхождений быть не должно
...
- markdownlint охватывает все markdown-файлы репозитория, кроме игнорируемых .gitignore (сторонние и генерируемые каталоги): npm-скрипт, CI обеих платформ
