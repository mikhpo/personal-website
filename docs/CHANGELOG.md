...
- Добавлен скилл fix-documentation: чеклист проверки текстов проекта - документация, комментарии в коде, докстринги (актуальное состояние, стиль, точность формулировок, ссылки, диаграммы)

- Сценарии развертывания переименованы: «Однопроектный сервер» и «Сервер с несколькими проектами» (были «облачный», «домашний», «сервер без Docker»); способ запуска (Compose/systemd) стал параметром, а не сценарием
- Из документации удалены история выбора прокси и разделы «осознанно не поддерживается»; правило «документация фиксирует только актуальное состояние» закреплено в AGENTS.md
- CI/CD и порядок работы с Taskfile перенесены из README.md в docs/CONTRIBUTING.md
- Диаграмма контейнеров упрощена до обобщенной картины
- Правило «дампы БД - единственные критичные для восстановления данные; две цели - рекомендация, offsite - требование» зафиксировано в backups.md
