...
Структура данного проекта была изменена для привнесения большой ясности в навигацию по модулям:
    ...
    personal-website/ <-- корневая директория репозитория
        ...
        personal_website/ <-- базовая директория проекта Django
            manage.py
            project/ <-- директория с настройками проекта
                __init__.py
                settings.py
                urls.py
                asgi.py
...
Отдельные скрипты расположены в директории `scripts` и предназначены для автоматического выполнения задач по обслуживанию проекта: например, создание бэкапов. Для постановки скриптов на запуск по расписанию можно воспользоваться скриптом `tools/cronjobs.sh`.

* `backup_database.py` - создать бэкап базы данных.
* `backup_storage.py` - создать бэкап загруженных файлов.
* `restore_database.py` - восстановить базу данных из бэкапа.
...
* `commit.sh` - отформатировать код и отправить коммит.
* `cronjobs.sh` - добавить задачи в cron.
* `deploy.sh` - подготовить первоначальное развертывание проекта.
* `makelink.sh` - создать символическую ссылку на node_modules.
* `restart.sh` - перезапустить Gunicorn, Nginx и PostgreSQL.
* `runtests.sh` - провести тесты.
* `update.sh` - получить изменения и перезапустить сервер.
