...
Также для выполнения всех тестов можно запустить bash-скрипт `run_tests.sh`. Скрипт `run_tests.sh` запускает тесты Node.js, тесты Pytest и создает отчет Coverage в формате HTML. Для запуска скрипта `run_tests.sh` необходимо воспользоваться командой:
