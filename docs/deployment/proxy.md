...
закрывается хостовым nginx + certbot с DNS-01 (раздел 2). Учтите
и другое следствие конфигурации labels: Traefik проксирует все запросы
в приложение и не умеет раздавать медиа с диска хоста, поэтому
продакшен с filesystem-хранилищем рассчитан на хостовой nginx.
