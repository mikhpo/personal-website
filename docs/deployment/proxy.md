...
Ось «Прокси»: слой терминирования HTTPS выбирается независимо от остальных
осей развертывания. Контракт приложения для этой оси - порт на 127.0.0.1
...
- симметрия осей: nginx одинаково проксирует контейнерные бэкенды
  (Docker Compose, docker run) и systemd-бэкенды - для него разницы нет,
  все они слушают 127.0.0.1:${DJANGO_PORT};
