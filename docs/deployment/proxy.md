...
Выбор nginx - решение пользователя от 2026-08-29, заменившее Traefik.
Причины. Во-первых, полноценная поддержка filesystem-хранилища требует
раздачи медиа именно прокси с локального диска, а Traefik файловый
сервер из коробки не умеет - его пришлось бы дополнять сайдкаром.
Во-вторых, схема nginx инвариантна к удостоверяющему центру: Let's
Encrypt, ACME-совместимый УЦ или ручной выпуск местным УЦ (например,
при недоступности Let's Encrypt) не меняют конфигурацию - меняются
только PEM-файлы. В-третьих, nginx одинаково проксирует контейнерные
бэкенды (Docker Compose, docker run) и systemd-бэкенды - все они слушают
127.0.0.1:${DJANGO_PORT}; мультипроектный хост обслуживает конфигурации
всех проектов одним экземпляром.
...
## 4. Пост-мортемы: nginx-в-контейнере и Traefik

### nginx-в-контейнере (коммит 692b9a94, 2026-08-15)

До замены прокси работал как кастомный образ nginx с cron и certbot внутри
контейнера: конфигурация генерировалась из шаблона через envsubst,
обновление сертификатов выполнял cron контейнера, а плагин
python3-certbot-nginx правил живой конфиг в рантайме. Схема была заменена
на Traefik. Причины, по которым ее тогда отвергли:
...
Пост-мортем относился именно к схеме «cron и certbot внутри
собственного образа». Пересмотр 2026-08-29: чистая схема раздела 3
(официальный образ nginx, certbot отдельным контейнером, webroot вместо
плагина, cron хоста, self-signed для dev) снимает все шесть причин -
по решению пользователя контейнерный слот занял nginx.

### Traefik (2026-08-15 - 2026-08-29)

Traefik заменил контейнерный nginx в 2026-08-15 и удален в 2026-08-29.
Что было: маршрутизация по Docker-labels, встроенный ACME-клиент
(TLS-ALPN-01), состояние в acme.json. Почему ушел: Traefik не умеет
раздавать файлы с локального диска, а полноценная поддержка filesystem-
хранилища требует раздачи медиа прокси - вариант со сайдкаром-файловым
сервером отвергнут пользователем как избыточный. Сравнение кандидатов
(Caddy, nginx, HAProxy, раздача приложением) по критериям «файлы
с диска, Let's Encrypt, минимум конфигурации, открытая лицензия,
устойчивость к смене УЦ» завершилось выбором nginx.

## 5. Неподдерживаемые варианты

- Traefik - см. пост-мортем в разделе 4.
- Общий прокси-стек сервера (Traefik в отдельном compose-проекте,
...
- Caddy, HAProxy и другие прокси-серверы - проект останавливается
  на одном инструменте (nginx).
