...
что и медиа, и раздается объектным хранилищем. Медиа при s3 отдает
объектное хранилище, а при filesystem - хостовой nginx читает файлы
с диска напрямую: в конфигурацию сайта включен location /media/,
указывающий на каталог `STORAGE_ROOT`, поэтому трафик медиа
не нагружает воркеры приложения (подробнее -
[proxy.md](./proxy.md)).
...
инфраструктурные сервисы (postgres, minio, traefik) подключаются
...
При работе с Compose помните об особенностях чтения .env: Compose
...
в контейнерных режимах, поэтому вариант рассчитан на слабое
и homelab-железо.
...
Traefik в контейнере не видит сокет хостовой операционной системы,
...
приложения; варианты и рецепты собраны в [proxy.md](./proxy.md):

- хостовой nginx + certbot - основной путь для мультипроектного
  хоста;
- Traefik в Docker Compose - вариант одиночного сервера «все
  в Docker».

Прокси не занимается статикой: о ней он ничего не знает (контракт -
в разделе 1). Единственное исключение - медиа в filesystem-режиме:
их nginx раздает с диска напрямую. Настройки безопасности за прокси
