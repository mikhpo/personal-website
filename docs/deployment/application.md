...
хостовой nginx - через location /media/ с alias на `STORAGE_ROOT`,
Traefik - через контейнер media (профиль compose), монтирующий каталог
хранилища read-only (подробнее - [proxy.md](./proxy.md)).
