...
публикуется только на 127.0.0.1: внешний трафик всегда проходит через
прокси-сервер. Флаг --add-host host.docker.internal:host-gateway
...
в контейнерных режимах, поэтому вариант подходит для слабого железа.
...
При переходе с контейнерного запуска в каталогах данных остаются файлы,
созданные контейнером от root: сервис под обычным пользователем не может
их открыть и завершается с PermissionError на файлах журналов.
Повторный запуск setup.sh передает каталоги данных сервис-пользователю
(create_data_directories выполняет chown рекурсивно).
...
данных; рецепт - в [database.md](./database.md), раздел 4 (БД
...
Альтернатива - перевести приложение на unix-сокет с активацией
сокетом (systemd socket activation). Вариант рассчитан на случай,
когда Gunicorn и nginx работают в одной операционной системе:
однопроектный сервер без Docker или сервер с несколькими проектами.
Он используется в продакшене проекта и может оставаться основным
режимом. Преимущества:

- бесшовные рестарты - на время перезапуска сервиса соединения
  копятся в backlog сокета, а не отвергаются;
- на сервере с несколькими проектами каждому приложению выделяется
  свой сокет, пул TCP-портов вести не нужно, nginx адресует сайты
  проектов путями сокетов;
- доступ ограничивается правами файла сокета (SocketGroup=www-data,
  SocketMode=0660), а не привязкой порта.

Когда вариант не подходит: контейнерным прокси сокет хоста не виден
(контейнер nginx работает в своем пространстве имен), поэтому вариант
практически означает «только хостовый nginx». Кроме того, для него
не выполняется контракт порта DJANGO_PORT.
...
(рецепты - в [proxy.md](./proxy.md)):

- nginx в Docker Compose (профиль nginx) с certbot отдельным
  контейнером - вариант однопроектного сервера;
- nginx на хосте с certbot - вариант сервера с несколькими проектами
  (общий nginx обслуживает сайты всех проектов).
