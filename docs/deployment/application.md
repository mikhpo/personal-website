...
Способ запуска приложения - Docker Compose, docker run без Compose или
systemd Gunicorn на хосте - выбирается независимо от остальных параметров
развертывания (база данных, хранилище, прокси, бэкапы). Автоматизация
первичной настройки: Docker Compose - scripts/docker/setup.sh,
systemd - scripts/server/setup.sh.
...
и эталонные сценарии развертывания - в [matrix.md](./matrix.md).
