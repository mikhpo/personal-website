...
В проекте уже настроен MinIO через Docker Compose для локальной разработки с S3. Сервис minio относится к профилю minio: `docker compose up -d minio` активирует профиль автоматически.
...
Обновление выполняется общим скриптом деплоя, который используется CI/CD (deploy-workflow) и пригоден для ручного запуска:

    bash scripts/deploy.sh

Скрипт выполняет резервное копирование файлового хранилища, затем по переменной окружения `DEPLOY_MODE` (см. [.env.example](./.env.example)) вызывает скрипт выбранного механизма развертывания. Значение `compose` (по умолчанию) обновляет файлы проекта из основной ветки репозитория, подготавливает каталоги, пересоздает контейнеры из свежих образов и очищает неиспользуемые образы. Значение `systemd` вызывает `scripts/server/deploy.sh` - развертывание в операционной системе хоста.
...
Резервное копирование БД и медиа реализовано приложением `backup` с management-командами: `backup`, `backup_db`, `backup_media` (с флагом `--verify` для проверки целей) создают копии, `restore_db`, `restore_media` - восстанавливают их. Команды запускаются хостовым скриптом `scripts/backup.sh` по расписанию cron. Цели бэкапов описываются в `.env` переменными `BACKUP_TARGET_<ИМЯ>` с префиксами `mc:` (MinIO Client) и `rclone:`.
