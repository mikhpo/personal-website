...
Эндпоинт `/health/` (проверочный SELECT 1) - единая точка проверки работоспособности приложения во всех режимах запуска.
...
Для фотографий реализован автоматический парсинг метаданных в формате EXIF. Миниатюры и превью генерируются фоновым воркером сразу после загрузки фотографий.
