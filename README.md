...
Сервисы postgres и minio в compose.yaml относятся к профилю dev и запускаются только при активном профиле: через переменную `COMPOSE_PROFILES=dev` в файле `.env` (рекомендуется для локальной разработки) или через флаг `docker compose --profile dev`. Без активного профиля поднимаются только сервисы website и nginx — это конфигурация продакшена, где используются внешние Managed PostgreSQL и S3.
