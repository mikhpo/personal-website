...
Каждый инфраструктурный сервис в compose.yaml относится к собственному профилю: postgres, minio, traefik. Профили активируются через переменную `COMPOSE_PROFILES=postgres,minio,traefik` в файле `.env` или через флаг `docker compose --profile <имя>`. Без активных профилей поднимается только сервис application — используйте этот режим, если база данных, объектное хранилище и обратный прокси предоставляются вне Docker Compose.
