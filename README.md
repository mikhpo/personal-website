...
3. **build-workflow** - запускается при пуше в ветку main
4. **deploy-workflow** - запускается после успешного релиза
...
* Автоматизированное тестирование и проверку качества кода при каждом пуше в ветку.
* Сборку и тестирование Docker-образа.
* Публикацию стабильного образа в Docker Hub и Yandex Cloud Container Registry после успешного слияния в `main` или вручную.
...
Условия запуска: при пуше в любую ветку, кроме основной (`main`).
...
Условия запуска: при пуше в основную ветку (`main`).
...
* Подключается к VPS серверу по SSH при помощи GitHub Actions
* Выполняет скрипт обновления приложения на сервере
