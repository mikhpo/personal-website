...
Параметры подключения — те же, что использует приложение (см. раздел «Настройка базы данных»). Утилиты libpq принимают их флагами (`-h`, `-p`, `-U`, `-d`). Пароль можно передать двумя способами:
...
Если дефолтный клиент старше сервера — вызвать совместимый бинарник явно (полный путь к нужной версии из установленного пакета postgresql-client).
...
Имя файла — по соглашению `<база>_<дата>.dump`, например `personal_website_2026-08-13.dump`:
    ...
    pg_dump -h <хост> -p <порт> -U <пользователь> -d <база> \
        -Fc -f personal_website_$(date +%F).dump
